...
uint8_t hr3_init( uint8_t address, dynamic_modes_t* dyn_modes )
{
   /* hr3_hal_init( address );
	...
	hr3_set_pdn_cycle_start_end( 5475, 64535 );
	hr3_set_prpct_count( 65535 );
...
uint8_t hr3_set_dynamic_settings( dynamic_modes_t* modes )
{
    ...
    buffer |= ( (uint32_t)modes->transmit << STT_DYNMC1 );
    buffer |= ( (uint32_t)modes->curr_range << STT_ILED_2X );
    buffer |= ( (uint32_t)modes->adc_power << STT_DYNMC2 );
    buffer |= ( (uint32_t)modes->clk_mode << STT_OSC_EN );
    buffer |= ( (uint32_t)modes->tia_power << STT_DYNMC3 );
    buffer |= ( (uint32_t)modes->rest_of_adc << STT_DYNMC4 );
    buffer |= ( (uint32_t)modes->afe_rx_mode << STT_PDNRX );
    buffer |= ( (uint32_t)modes->afe_mode << STT_PDNAFE );
...
uint8_t hr3_set_transmit_enable( transmitter_t transmit )
{
    ...
    buffer |= ( (uint32_t)transmit << STT_DYNMC1 );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_current_range( led_current_range_t curr_range )
{
    ...
    buffer |= ( (uint32_t)curr_range << STT_ILED_2X );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_adc_power( adc_pwer_t adc_power )
{
    ...
    buffer |= ( (uint32_t)adc_power << STT_DYNMC2 );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_clock_mode( clk_mode_t clk_mode )
{
    ...
    buffer |= ( (uint32_t)clk_mode << STT_OSC_EN );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_tia_power( tia_pwer_t tia_power )
{
    ...
    buffer |= ( (uint32_t)tia_power << STT_DYNMC3 );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_rest_of_adc( rest_of_adc_t rest_of_adc )
{
    ...
    buffer |= ( (uint32_t)rest_of_adc << STT_DYNMC4 );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_afe_receive( afe_rx_mode_t afe_rx_mode )
{
    ...
    buffer |= ( (uint32_t)afe_rx_mode << STT_PDNRX );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_afe_mode( afe_mode_t afe_mode )
{
    ...
    buffer |= ( (uint32_t)afe_mode << STT_PDNAFE );
    temp[2] |= buffer;
    temp[1] |= buffer >> 8;
    temp[0] |= buffer >> 16;
...
uint8_t hr3_set_clkout_div( bool enable, uint8_t div )
{
    ...
    if( enable )
    {
        temp[1] = ( (uint32_t)1 << CLKOUT_EN );
        temp[2] = ( (uint32_t)div << CLKOUT_DIV );
        hr3_hal_write( &reg, temp, 3 );
    }
    else
    {
...
uint8_t hr3_set_offdac_settings( offdac_settings_t* settings )
{
    ...
    buffer |= ( (uint32_t)settings->offdac_pol_led2 << POL_OFFDAC_LED2 );
    buffer |= ( (uint32_t)settings->offdac_set_led2 << I_OFFDAC_LED2 );
    buffer |= ( (uint32_t)settings->offdac_pol_amb1 << POL_OFFDAC_AMB1 );
    buffer |= ( (uint32_t)settings->offdac_set_amb1 << I_OFFDAC_AMB1 );
    buffer |= ( (uint32_t)settings->offdac_pol_led1 << POL_OFFDAC_LED1 );
    buffer |= ( (uint32_t)settings->offdac_set_led1 << I_OFFDAC_LED1 );
    buffer |= ( (uint32_t)settings->offdac_pol_amb2 << POL_OFFDAC_LED3 );
    buffer |= ( (uint32_t)settings->offdac_set_amb2 << I_OFFDAC_LED3 );
